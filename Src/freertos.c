...
__weak void check_online(void *argument)
{
  ...
  while (1)
  {
    for (int i = 0; i < 8; i++)
    {
      ...
      if (online_flag & motor_bit) // ��⵽�������
      {
        if (Motor_online.motor_online & motor_bit) // �ѵǼ����ߣ����ü�ʱ��
        {
        ...
        else // δ�Ǽ����ߣ����Ϊ����
        {
          ...
          if (!chassis_flag &&(
              ((motor_bit == 0x80 && Motor_online.motor_online & 0x0F) ||
               (motor_bit & 0x0F && Motor_online.motor_online & 0x80)))) // ��⵽c�����ߣ�������������
          {
            chassis_flag = 1;
            LEDHandle = osThreadNew(led_task, NULL, &LED_attributes);
            ChassisHandle = osThreadNew(chassis_task, NULL, &Chassis_attributes);
      ...
      else if (Motor_online.motor_online & motor_bit) // �ѵǼ����ߵ�δ�յ������ź�
      {
        // ���Ӽ�ʱ������ʱ����Ϊ����
        if (++Motor_online.motor_ticks[i] == CHECK_ONLINE_TIMEOUT_COUNT)
        {
          ...
          // ����c���������
          if (((Motor_online.motor_online & 0x0F) != 0x0F) || !(Motor_online.motor_online & 0x80)) // c�����߻��ĸ����Ӷ�����
          {
            // ���õ���Ϊ������ģʽ
